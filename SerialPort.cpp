	...
	if (SetCommTimeouts(m_hComm, &m_CommTimeouts))
	{
		if (SetCommMask(m_hComm, dwCommEvents))
		{
			if (GetCommState(m_hComm, &m_dcb))
			{
				m_dcb.EvtChar = 'q';
				m_dcb.fBinary = TRUE;
				m_dcb.fDtrControl = DTRCtrl;
				m_dcb.fRtsControl = RTSCtrl;
...
UINT CSerialPort::CommThread(LPVOID pParam)
{
	...
	COMSTAT comstat;
	BOOL  bResult = TRUE;
	port->m_dwLastTime = timeGetTime();
	port->m_bHasRecieved = FALSE;
			...
			default:
				{
					...
					port->ClosePort();
					port->m_pOwner->SendMessage(WM_COMM_RW_ERROR, 0, 0);
...
//				continue;
		...
		if(Event & EV_TXEMPTY)
			PostMessage(port->m_pOwner->m_hWnd, WM_COMM_TXEMPTY_DETECTED, 0, 0);
//		SetEvent(port->m_ov.hEvent);
		...
		if(port->m_dwTimerOut && port->m_bHasRecieved)
		{
			...
			if((dwCurrtime - port->m_dwLastTime) > port->m_dwTimerOut)
			{
				port->m_bHasRecieved = FALSE;
				::SendMessage(port->m_pOwner->m_hWnd, WM_COMM_TIMEOUT, (WPARAM) 0, (LPARAM) port->m_nPortNr);
			}
		}
		switch (Event)
		{
		...
		case 1:	// read event
			{
				...
				if(WAIT_OBJECT_0 != WaitForSingleObject(port->m_hWriteEvent, 0))
					break;
			}
		case 2: // write event
			{
...
//	::SendMessage((port->m_pOwner)->m_hWnd, WM_COMM_TXEMPTY_DETECTED,BytesSent,(LPARAM) port->m_nPortNr);
//	::PostMessage((port->m_pOwner)->m_hWnd, WM_COMM_TXEMPTY_DETECTED,0,(LPARAM) port->m_nPortNr);
}
	...
	for (;;)
	{
		...
		if (bRead)
		{
			...
			bResult = ReadFile(port->m_hComm,		// Handle to COMM port
							   RXBuff,//&cData,				// RX Buffer Pointer
							   256,//1,					// Read one byte
...
/*
			...
			if(i>=256)
			{
				::SendMessage((port->m_pOwner)->m_hWnd, WM_COMM_RXCHAR,
					(WPARAM) RXBuff, (LPARAM)MAKELONG(i, port->m_nPortNr));
				i=0;
			}
*/
			port->m_dwLastTime = timeGetTime();
			port->m_bHasRecieved = TRUE;
			::SendMessage((port->m_pOwner)->m_hWnd, WM_COMM_RXCHAR,
...
//	memset(m_szWriteBuffer, 0, m_nWriteBufferSize);
	...
	if (m_nWriteSize > m_nWriteBufferSize)
		m_nWriteSize = m_nWriteBufferSize;
